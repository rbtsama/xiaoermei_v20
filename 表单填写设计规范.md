...
```vue
<a-checkbox-group v-model="selectedValues" style="width: 100%">
  <a-row :gutter="[16, 12]">
    <a-col :span="6" v-for="item in options" :key="item">
...
1. **Row/Col布局**：使用Ant Design标准栅格系统
   - gutter: [16, 12]（列间距16px，行间距12px）
   - 每个checkbox包在a-col中
   - 完美对齐，无需hack

2. **强制去除margin**：checkbox-wrapper的margin必须为0
   - 不依赖display: contents
   - 不需要覆盖相邻选择器

3. **卡片样式**：边框+padding+背景色
   - width: 100%填满col宽度
   - 边框：1px solid #e2e8f0
   - 圆角：6px
   - 内边距：10px 12px
...
**所有提示文字必须完全一致**（field-hint、upload-hint、error-hint）：

```less
.field-hint,
.upload-hint {
  font-size: 12px;          // @font-size-xs - 固定12px
  color: #666666;           // @text-secondary - 固定灰色
  margin-top: 4px;          // 固定4px
  line-height: 1.4;         // 固定1.4
}

.error-hint {
  font-size: 12px;          // @font-size-xs - 固定12px
  color: #ef4444;           // @error-color - 固定红色
  margin-top: 4px;          // 固定4px
  line-height: 1.4;         // 固定1.4
}
```
...
**❌ 常见错误**：
- 缺少line-height（导致默认值不一致）
- margin-top不是4px（如16px、12px等）
- line-height是1.6而不是1.4
- font-size是13px而不是12px
- 有多余的margin-bottom

**✅ 检查方法**：
```bash
# 检查所有提示文字样式定义
grep -A 5 "\.field-hint {" components/*.vue
grep -A 5 "\.upload-hint {" components/*.vue
grep -A 5 "\.error-hint {" components/*.vue

# 确认每个都有：
# - font-size: @font-size-xs (12px)
# - color: @text-secondary (#666) 或 @error-color (#ef4444)
# - margin-top: 4px
# - line-height: 1.4
```
...
```less
:deep(.ant-form-item-label) {
  font-weight: 400;              // 固定正常字重，不加粗
  color: rgba(0,0,0,0.9);       // 固定黑色
  text-align: left;              // 固定左对齐
}
...
.upload-label {                  // Tab4使用
  font-size: 14px;
  font-weight: 600;              // 加粗（图片标签特殊）
  color: rgba(0,0,0,0.9);
}
```

**❌ 常见错误**：
- font-weight是500或600（应该是400）
- color是#2563eb蓝色（应该是黑色）
- 缺少text-align: left
...
// 卡片padding
:deep(.ant-card-head) {
  padding: 16px 24px;            // 固定
}

:deep(.ant-card-body) {
  padding: 32px 24px;            // 固定
}
```

**❌ 常见错误**：
- 字段间距不是24px（如20px）
- 卡片内容padding不是32px 24px
- 分类间距不是32px
