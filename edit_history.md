...
**包含子菜单（8个）**:
1. **基本信息** - `/merchant-backend/store/basic-info`
   - 门店名称、地址、联系方式等基本信息

2. **政策相关** - `/merchant-backend/store/policy`
   - 入住离店时间、取消政策、押金政策等

3. **门店设施** - `/merchant-backend/store/facilities`
   - WiFi、停车场、健身房等设施配置

4. **周边信息** - `/merchant-backend/store/surrounding`
   - 地铁站、景点、商圈等周边信息

5. **早餐政策** - `/merchant-backend/store/breakfast`
   - 早餐类型、时间、费用等配置

6. **加床政策** - `/merchant-backend/store/extra-bed`
   - 加床费用、儿童政策等配置

7. **门店图片** - `/merchant-backend/store/images`
   - 门店外观、大堂、设施等图片管理

8. **支付结算** - `/merchant-backend/store/payment-settlement`
   - 支付方式、结算周期、账户信息等
...
**默认展开配置**:
- 在 `expandedMenus` state 中添加 `'门店信息': true`
- 确保菜单默认展开，用户可直接看到子菜单
...
✅ **便于后续开发**：
- 菜单路由已定义
- 路径规范统一（/merchant-backend/store/xxx）
- 可按优先级逐步开发各页面

**路由路径规范**:
- 前缀: `/merchant-backend/store/`
- 基本信息: `basic-info`
- 政策相关: `policy`
- 门店设施: `facilities`
- 周边信息: `surrounding`
